module.exports = function extractUSDTInfo(exchange, response) {
  ...
  if (exchange.symbolPath) {
      const dataArray = exchange.symbolPath.split('.').reduce((obj, key) => obj && obj[key], response.data);
      usdtInfo = Array.isArray(dataArray) ? dataArray.find(item => item.asset === 'USDT') : dataArray;
  } else {
